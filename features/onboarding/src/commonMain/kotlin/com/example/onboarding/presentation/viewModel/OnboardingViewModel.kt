...
import com.example.onboarding.presentation.PAGER_SIZE
...
class OnboardingViewModel : ViewModel(), OnboardingAction {
    ...
    override fun sendAction(action: OnboardingAction.Action) {
        when (action) {
            OnboardingAction.Action.GoToInformativeScreen -> {
                _effect.value = OnboardingEffect.GoToOnboarding
                _state.value = OnboardingState.Initial(
                    checkNotNull(_state.value).uiModel.copy(
                        currentStep = 0
                    )
                )
            }

            is OnboardingAction.Action.NavigateStep -> {
            ...
            OnboardingAction.Action.ClickCreateAccountButton -> {
                _effect.value = OnboardingEffect.GoToCreateAccount
            }

            OnboardingAction.Action.ClickLoginButton -> {
                _effect.value = OnboardingEffect.GoToLogin
            }

            OnboardingAction.Action.Initialize -> {
